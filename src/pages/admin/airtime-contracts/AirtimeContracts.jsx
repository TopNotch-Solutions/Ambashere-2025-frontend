...
const AdminAirtimeContracts = () => {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      const analyticsRes = await axiosInstance.get(
        "/contracts/submissions/analytics"
      );
      setAnalytics(analyticsRes.data);
    } catch (error) {
      console.error("Error fetching airtime submission analytics:", error);
      setAnalytics(null);
    }
  }, []);
  ...
  return (
    <Box m="20px" className="handset-simulator-page admin-dashboard-page">
      <div className="handset-hero mb-4">
        <div>
          ...
          <p className="handset-subtitle mb-0">
            Review airtime contract submissions, assign contracts to yourself when
      ...
      {analytics?.byTransactionType?.length > 0 && (
        <Box
          className="shadow admin-dashboard-card handset-form-card mb-4"
          sx={{ p: 2 }}
        >
          <h6 className="summary-title mb-3">Submissions by Transaction Type</h6>
          <div className="row g-2">
            {analytics.byTransactionType.map((item) => (
              <div
